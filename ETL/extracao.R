...
colnames(sinistrosRecife2019Raw) <- tolower(names(sinistrosRecife2019Raw))

colunas_iguais <- intersect(
  names(sinistrosRecife2019Raw), names(sinistrosRecife2020Raw))

colunas_iguais <- intersect(
  colunas_iguais, names(sinistrosRecife2021Raw))

#A função pipe %>% usa o valor resultante da expressão do lado esquerdo como primeiro argumento da função do lado direito.
...
# modifica situação do sinistro de texto para fator
...
rm(list = ls()[!ls() %in% c("sinistrosRecifeRaw", "naZero")])

saveRDS(sinistrosRecifeRaw, "bases_tratadas/sinistrosRecife.rds")

write.csv2(sinistrosRecifeRaw, "bases_tratadas/sinistrosRecife.csv")

# A base de dados sinistrosRecife2019Raw estava consumindo 4200512